...
// 通过 Vite 读取 workspace/reports 下的 md 文件
// 注意：Vite 只能在构建时扫描静态路径，运行时无法读取磁盘
const files = import.meta.glob('/workspace/reports/*.md', { as: 'raw', eager: true });
...
function buildFileUrls(rawFiles: Record<string, string>): Record<string, string> {
  ...
  for (const path in rawFiles) {
    // 直接使用路径作为 URL（Vite 开发服务器会处理这些路径）
    urls[path] = path;
  }
  return urls;
}
...
export type ReportEntry = ReportMeta & { content: string };
...
export function loadReports(): ReportEntry[] {
  ...
  const list: ReportEntry[] = Object.entries(files).map(([path, content]) => {
